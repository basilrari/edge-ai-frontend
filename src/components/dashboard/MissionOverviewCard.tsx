...
function StatBlock({
  ...
  return (
    <div className="rounded-md border border-dash-border bg-dash-bg/60 px-3 py-2">
      ...
      {sub ? <p className="text-[10px] text-dash-muted">{sub}</p> : null}
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        />
        <StatBlock label="Est. Time" value={`${stats.estTimeMin} min`} />
        <StatBlock
          label="Max Altitude"
          value={`${stats.maxAltitudeM.toFixed(0)} m`}
        />
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto rounded-md border border-dash-border bg-dash-bg/40">
